import React, { lazy, useContext, useState, memo, useCallback } from 'react'
...
const ProductItem = memo(function ProductItem({ data, onAddToCart }) {
  return (
        ...
        <div className='parent mt-50 mb-16 grid grid-cols-3 sm:w-[80%] mx-auto xl:w-[52%] xl:mx-0 bg-gray-100 border border-gray-200 dark:bg-gray-800'>
          {
            filteredItems.length === 0 ? (
              ...
              filteredItems.map(data => (
                <ProductItem key={data.id} data={data} onAddToCart={handleAddToCart} />
              ))
            )
          }
        </div>
